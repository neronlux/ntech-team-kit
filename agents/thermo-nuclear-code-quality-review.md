...
- Determine the base branch (`main` or `origin/main` in most repos).
...
- For each meaningfully changed file, read its full current content (use `Read` or similar file tools).
- Then proceed to the review using the rubric below.

You have permission for `git fetch*`, `git diff*`, and `git log*`. Use them.
...
For large diffs, a parent agent can run two `Task` calls in parallel (`subagent_type: "explore"` + `"general"`) to collect `git diff <base>...HEAD` and full file contents, then hand the labeled output to this agent. This avoids duplicate work but is not required for normal use.
